...
class ShortCodeBlock extends \WP_Super_Duper {
	...
	public function __construct() {
		$options = array(
			...
			'block-output'   => array( // the block visual output elements as an array
				array(
					'element' => 'p',
					'title'   => \__( 'Placeholder', TDM_TEXTDOMAIN ),
					'class'   => '[%className%]',
			...
			'name'           => \__( 'Hello World', TDM_TEXTDOMAIN ),
			// the name of the widget/block
			'widget_ops'     => array(
				'classname'   => 'hello-world-class',
				// widget class
				'description' => \esc_html__( 'This is an example that will take a text parameter and output it after `Hello:`.', TDM_TEXTDOMAIN ),
				// widget description
			),
			...
			'arguments'      => array( // these are the arguments that will be used in the widget, shortcode and block settings.
				'after_text' => array( // this is the input name=''
					'title'       => \__( 'Text after hello:', TDM_TEXTDOMAIN ),
					// input title
					'desc'        => \__( 'This is the text that will appear after `Hello:`.', TDM_TEXTDOMAIN ),
					// input description
					'type'        => 'text',
	...
	public function initialize() {
		...
		'widgets_init',
		static function() {
			\register_widget( 'Tasmota_Device_Manager\Internals\ShortCodeBlock' );
		}
		);
		*/
	}
